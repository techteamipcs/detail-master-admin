...
@Component({
  ...
  templateUrl: './add-socialactivity.component.html',
  styleUrls: ['./add-socialactivity.component.scss']
})
export class AddSocialactivityComponent implements OnInit {

  @ViewChild('uploader', { read: ElementRef }) fileInput: ElementRef;
	// File Upload
	options: UploaderOptions;
	uploadInput: EventEmitter<UploadInput>;
	socialactivityImage: any;
	socialactivityVideo: any;
	imagePath: any;
	imageArr: any = [];
	// Data Assign
	artData: any;
	countryData: any;
	addsocialactivityForm: FormGroup;
	throw_msg: any;
	...
	constructor(
		...
		this.addsocialactivityForm = this.formBuilder.group({
	...
	public hasError = (controlName: string, errorName: string) => {
		return this.addsocialactivityForm.controls[controlName].hasError(errorName);
	};
	...
	patchingdata(id: any) {
		let obj = { id: id };
		this.socialactivityService.getSocialactivityWithId(obj).subscribe(
			(response) => {
	...
	onSubmit() {
		this.submitted = true;
		let obj = this.addsocialactivityForm.value;
		let id = this.id;
		...
		if (!this.isEdit) {
			...
			this.socialactivityService.editSocialactivitydata(obj, id).subscribe(
				(response) => {
					if (response.code == 200) {
						this.throw_msg = response.message
						this.msg_success = true;
