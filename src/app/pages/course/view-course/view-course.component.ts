...
export class ViewCourseComponent implements OnInit {
  ...
  selectedCourse : any;
	...
	get_CourseData() {
		...
		this.courseservice.getCourseDetails(obj).subscribe(
			(response) => {
  ...
  importCourse()
  {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.courseservice.importallCourse(this.currentFile).subscribe(
            (response)=> {
              if (response.code == 200) 
              {
                console.log('file uploaded sucessfully');
                this.toastr.successToastr("course Image Imported sucessfully");
                setTimeout(()=>{                           
  ...
  exportCourse()
  {
    if(confirm("Are you sure to Export this Course"))
    {
      var obj = {};
      this.courseservice.exportCourse(obj).subscribe(
        (response)=> {
          if (response) 
          {   
	...
	deleteAllCourses()
  {
    if(confirm("Are you sure to delete All Courses"))
    {
      var mylist = {};
      this.courseservice.deleteallcourses(mylist).subscribe(
        (response)=> {
          if (response.code == 200) 
          {   
            this.get_CourseData();
  ...
  deleteCourse() {
		if (this.selectedCourse) {
			var mylist = { id: this.selectedCourse._id };
			this.courseservice.deletecourse(mylist).subscribe(
				(response) => {
					if (response.code == 200) {
						this.get_CourseData();
