...
export class AddCourseComponent implements OnInit {
  ...
  @ViewChild('uploader', { read: ElementRef }) fileInput: ElementRef;
	...
	patchingdata(id: any) {
		let obj = { id: id };
		this.courseService.getCourseWithId(obj).subscribe(
			(response) => {
	...
	onSubmit() {
		this.submitted = true;
		let obj = this.addcourseForm.value;
		let id = this.id;
		...
		if (!this.isEdit) {
			this.courseService.addCourse(obj).subscribe(
				(response) => {
					if (response.code == 200) {
			...
			this.courseService.editCoursedata(obj, id).subscribe(
				(response) => {
					if (response.code == 200) {
						this.throw_msg = response.message
						this.msg_success = true;
	...
	onUploadOutput(output: UploadOutput, typeofImage): void {
		if (output.type === 'allAddedToQueue') {
			const event: UploadInput = {
				type: 'uploadAll',
				url: environment.baseUrl + '/api/course/addimage',
				method: 'POST',
				data: {},
			};
				...
				this.mediaService.editMediadata(obj, id).subscribe(
					(response) => {
						if (response.code == 200) {
							...
							this.mediaData.src = response.result.src;
							if (this.courseData.media_data && this.courseData.media_data.length > 0) {
								this.patchingdata(this.id);
							}
							this.modalService.dismissAll();
						} else {
	...
	deleteMediaData() {
		if (this.deletedMediaData) {
			...
			this.courseService.deleteMediaData(mylist).subscribe(
				(response) => {
					if (response.code == 200) {
						this.modalService.dismissAll();
	...
	deleteMediaFile() {
		if (this.isUploaded && this.deletedMediaFile && this.deletedMediaFile.length > 0) {
			...
			this.courseService.deletefile(obj).subscribe(
				(response) => {
					if (response.code == 200) {
						this.isUploaded = false;
						this.mediaFile = '';
